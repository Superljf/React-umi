...
import { renderAuthorityDom } from '../../utils/authority';
...
const itemRender = (route, params, routes, paths) => {
  ...
  const str = '/trainingDetail/';
  if(route.breadcrumbName === '培训项目详情' && last){
    localStorage.setItem('trainingId',route.path.substring(route.path.indexOf(str) + str.length));
  }
  if (!last) {
    let flag = 0;
    if(route.breadcrumbName === '培训项目详情'){
      flag = 1;
    }
    return (
      <React.Fragment>       
        {route.breadcrumbName === '教师培训' && <Link to='/'>{route.breadcrumbName}</Link>}
        {route.breadcrumbName !== '教师培训' && <Link to={flag === 1 ? route.path.replace(route.path.substring(route.path.indexOf(str) + str.length),localStorage.getItem('trainingId')) : route.path}>{route.breadcrumbName}</Link>}
      </React.Fragment>
    );
  }

...
const responseStatus = (response) => {
  response.clone().json().then((json) => {
    const { status, message, action } = json;
    if (status !== '200') {
    //  const errortext = codeMessage[status];
      ...
      if(status === '401'){
        window.location.href = '/home/2.0/';
      }
      if (status === '403') {
        router.push(`/exception/403`);
      }
      if ((action === 'inquireTeacherTrainingDetailTenant' || action === 'inquireTeacherTrainingHomeworkDetailTenant') && status === '404'){
        router.push(`/exception/delete`);
      }
    }
  });
  return response;
...
export default function request(url, option) {
  ...
  if(ind < 0){
    rewriteUrl = `/${requestPath.project}/api/${requestPath.version1}/${rewriteUrl}`;
  }
  ...
  return fetch(rewriteUrl, newOptions)
    .then(checkStatus)
    .then(responseStatus)

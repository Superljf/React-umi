...
/**
 * 对象数组转map
 * @param datas [{id: '1', name: '小明', age: '18'},{id: '2',name: '张三', age: '19'}]
 * @param key1 以某个属性作为key（必填）
 * @param key2 key1+key2复合主键key（可选）
 * @returns {Map<any, any>}
 */
 ...
 * @param to 成功后跳转地址
 */
export function hideSubmitModal(modal, flag, successContent, failContent, to ){
  let time = 1000;
  if(!flag){
    time = 3000;
  }
...
export function historyBack(props){
  const { history } = props;
  history.goBack();
}
 ...
 * @returns {any}
 */
export function deepCloneJStr(obj) {
...
export function submitOnCancel(){
  Modal.confirm({
    onOk: () => { router.goBack();},
    title: '取消',
    content: '是否确定取消？',
